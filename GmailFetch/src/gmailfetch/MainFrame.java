                ...
                if(messages[i].getSubject() != null)
                    if ((messages[i].getSubject().contains("Digital Appointment Request"))&& messages[i].getReceivedDate().compareTo(datum)>0)
                    {
                        ...
                        else
                        {
                            ...
                            client_gui.sendEmail(sender, "Appointment declined: Appointment time unavailable", content, email, email, password);
                        }
                    }
            }
        }
            ...
            for ( int l = 0; l < k; l++) {
                ...
                if(JOptionPane.showConfirmDialog(null, content, "Appointment Request", JOptionPane.YES_NO_OPTION)==JOptionPane.YES_OPTION)
                {
                    ...
                    try {
                        ...
                        client_gui.sendEmail(sender, "Appointment confirmed", content, email, email, password);
                    } catch (IOException ex) {
                        Logger.getLogger(MainFrame.class.getName()).log(Level.SEVERE, null, ex);
                    }
                }
                else
                {
                    String sender = InternetAddress.toString(fmessages[l].getFrom());
                            sender = sender.substring(sender.indexOf("<")+1);
                            sender = sender.substring(0,sender.length()-1);
                    client_gui.sendEmail(sender, "Appointment declined", content, email, email, password);
                }
            }
        }
        else
              ...
              public void actionPerformed(ActionEvent arg0)
                 {
                     ...
                     else
                     {
                        ...
                        if(indxm.length()==1)
                         ...
                         try
                         {
                         client_gui.sendEmail(txtAttendee.getText(), "Digital Appointment Request", content, email, email, password);
